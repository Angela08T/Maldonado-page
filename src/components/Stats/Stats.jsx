...
const socials = [
  ...
  { label: 'Club de Fans', handle: '@somos_fans_',         img: somosFans,                                  href: 'https://www.tiktok.com/@somos_fans_' },
]
        ...
        <div className={styles.socialIcons}>
          {socials.map(s => (
            <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" className={styles.socialIcon}>
              {s.img ? (
                <img src={s.img} alt={s.label} className={styles.socialImg} />
              ) : (
                <div className={styles.socialCircle} style={{ background: s.color, boxShadow: `0 6px 24px ${s.glow}` }}>
                  <svg width="36" height="36" viewBox="0 0 24 24">{s.icon}</svg>
                </div>
              )}
